...
export const SCRATCH_GATE_ALGORITHM_VERSION = 5;
...
function normalizeEvent(value, index, sourceScale, outputScale) {
  ...
  if (value.type.startsWith("scratch-")) {
    if (value.type === "scratch-preset") {
      ...
      if (value.type === "scratch-end") {
        event.resumePlayback = value.resumePlayback !== false;
        event.cancelled = Boolean(value.cancelled);
      }
    }
