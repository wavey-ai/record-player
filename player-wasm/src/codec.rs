...
use encodec_rs::format::{
    ecdc_chunk_layout_from_bundle, ecdc_chunk_layout_from_metadata, ecdc_frame_ranges,
    ecdc_lm_frame_length, validate_metadata, EcdcChunkLayout, EcdcMetadata,
...
const OPUS_CHUNK_CACHE_VERSION: &str = "bitneedle-opus-chunk-cache-v3";
...
pub fn wasm_ecdc_chunk_layout_from_metadata(
    ...
    let _: EcdcMetadata = read_ecdc_header(&mut Cursor::new(payload)).map_err(to_js_error)?;
    let _ = chunk_count;
    let layout: EcdcChunkLayout = ecdc_chunk_layout_from_bundle(&meta).map_err(to_js_error)?;
...
pub fn wasm_ecdc_crop_decoded_owned_audio(
    ...
    if decoded_samples < required {
        return Err(to_js_error(format!(
            "decoded audio has {decoded_samples} samples per channel but requires {required}"
        )));
    }
...
pub fn wasm_record_playback_metadata_from_header_json(header_json: &str) -> String {
    ...
    let payload_container = arbitrary
        .get("payloadContainer")
        .or_else(|| descriptor.get("payloadContainer"))
        .and_then(Value::as_str)
        .unwrap_or("");
    let track_listing = arbitrary
        .get("trackListing")
        .or_else(|| descriptor.get("trackListing"))
        .cloned()
        .unwrap_or_else(|| Value::Array(vec![]));
    let dummy = arbitrary
        .get("dummySpiralRegions")
        .or_else(|| descriptor.get("dummySpiralRegions"))
        .cloned()
        .unwrap_or_else(|| Value::Array(vec![]));
...
pub fn wasm_resolve_playback_payload_metadata_json(
    ...
    let payload_container = header
        ...
        .or_else(|| payload.get("payloadContainer").and_then(Value::as_str))
        .unwrap_or("")
        .to_uppercase();
    let tracks = header
        .get("trackListing")
...
fn create_player_ecdc_cache_proof_context(ecdc: &[u8]) -> Option<EcdcCacheProofContext> {
    ...
    while offset + 8 <= ecdc.len() {
        let payload_len =
            u32::from_be_bytes(ecdc.get(offset..offset + 4)?.try_into().ok()?) as usize;
...
fn validate_encodec_lm_metadata(meta: &OnnxFrameBundleMetadata) -> Result<(), String> {
    meta.lm_dim().map_err(|e| e.to_string())?;
    meta.lm_num_layers().map_err(|e| e.to_string())?;
    meta.lm_past_context().map_err(|e| e.to_string())?;
    if meta.lm_cardinality() == 0 {
        return Err("LM cardinality must be non-zero".into());
    }
    Ok(())
}
fn encodec_probability_columns_from_logits(
    ...
    if logits.len() != card * codebooks {
        return Err("LM logits shape mismatch".into());
    }
    let mut pdf = vec![0.0; card * codebooks];
    let mut quantized = vec![0.0; card];
    let mut probs = vec![0.0; card];
    let uniform = 1.0 / card as f64;
    let threshold = 0.25 / DEFAULT_FP_SCALE as f64;
    let step = meta.lm_entropy_logit_step();
    for codebook in 0..codebooks {
        let (mut max_value, mut min_value) = (f64::NEG_INFINITY, f64::INFINITY);
        for bin in 0..card {
            let q = ((logits[bin * codebooks + codebook] as f64 / lm_tau) / step + 0.5
                - 2_f64.powi(-40))
            .floor()
                * step;
            quantized[bin] = q;
        ...
        let mut denom = 0.0;
        for bin in 0..card {
            probs[bin] = (quantized[bin] - max_value).exp();
            denom += probs[bin];
        }
        ...
        let (mut max_pdf, mut min_pdf) = (0.0_f64, f64::INFINITY);
        for p in &mut probs {
            *p /= denom;
            max_pdf = max_pdf.max(*p);
            min_pdf = min_pdf.min(*p);
        }
...
mod tests {
    ...
    fn ecdc_chunk_cache_key_changes_with_binding_and_chunk() {
        ...
        let scoped = wasm_ecdc_chunk_cache_key(
            CHUNK_PAYLOAD_ONE,
